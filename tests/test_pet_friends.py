...
def test_post_api_create_pet_simple(name='Фил', animal_type='кот', age = '4'):
...
def test_post_api_pets_set_photo(pet_photo = 'images/Cat.jpg'):
...
def test_delete_pet_with_notvalid_auth_key():
   ...
   # Получаем ключ auth_key и список своих питомцев
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Дик", "собака", "3", "images/Dog.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_update_pet_info_invalid_auth_key(name='Фил', animal_type='кот', age=4):
